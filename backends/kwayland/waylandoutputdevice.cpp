...
void WaylandOutputDevice::updateKScreenOutput(OutputPtr &output)
{
    ...
    output->setAutomaticMaxBitsPerColor(m_autoMaxBpc);
...
void WaylandOutputDevice::kde_output_device_v2_max_bits_per_color(uint32_t max_bpc, uint32_t autoValue, uint32_t min_value, uint32_t max_value)
{
    if (max_bpc == 0) {
        m_maxBpc.reset();
    } else {
