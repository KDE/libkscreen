                                      ...
                                      const QMap<int, WaylandOutputDevice *> &outputMap)
{
    ...
    if (version >= KDE_OUTPUT_CONFIGURATION_V2_SET_CUSTOM_MODES_SINCE_VERSION && m_customModes != output->customModes()) {
        ...
        for (const auto &mode : modes) {
            kde_mode_list_v1_set_resolution(list, mode.size.width(), mode.size.height());
            kde_mode_list_v1_set_refresh_rate(list, std::round(mode.refreshRate * 1000));
            kde_mode_list_v1_set_reduced_blanking(list, mode.flags & ModeInfo::Flag::ReducedBlanking ? 1 : 0);
            kde_mode_list_v1_add_mode(list);
        }
        wlConfig->set_custom_modes(object(), list);
        kde_mode_list_v1_destroy(list);
        changed = true;
    }
